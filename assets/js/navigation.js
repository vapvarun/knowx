...
if ('loading' === document.readyState) {
    ...
    document.addEventListener('DOMContentLoaded', knowx_initNavigation);
} else {
    // The DOM has already been loaded.
    knowx_initNavigation();
}

// Initiate the menus when the DOM loads.
function knowx_initNavigation() {
    knowx_initNavToggleSubmenus();
    knowx_initNavToggleSmall();
}
 ...
 */
function knowx_initNavToggleSubmenus() {
    ...
    for (let i = 0; i < navTOGGLE.length; i++) {
        knowx_initEachNavToggleSubmenu(navTOGGLE[i]);
    }
}
 ...
 */
function knowx_initEachNavToggleSubmenu(nav) {
    // Get the submenus.
    ...
    var dropdownButton = knowx_getDropdownButton();
    ...
    for (let i = 0; i < SUBMENUS.length; i++) {
        ...
        thisDropdownButton.addEventListener('click', (e) => {
            knowx_toggleSubMenu(e.target.parentNode);
        });

        // Clean up the toggle if a mouse takes over from keyboard.
        parentMenuItem.addEventListener('mouseleave', (e) => {
            knowx_toggleSubMenu(e.target, false);
        });
        ...
        parentMenuItem.querySelector('a').addEventListener('focus', (e) => {
            ...
            for (let j = 0; j < parentMenuItemsToggled.length; j++) {
                knowx_toggleSubMenu(parentMenuItemsToggled[j], false);
            }
        });
        ...
        SUBMENUS[i].addEventListener('keydown', (e) => {
            ...
            if (knowx_keymap.TAB === e.keyCode) {
                if (e.shiftKey) {
                    // Means we're tabbing out of the beginning of the submenu.
                    if (knowx_isfirstFocusableElement(e.target, document.activeElement, focusSelector)) {
                        knowx_toggleSubMenu(e.target.parentNode, false);
                    }
                    // Means we're tabbing out of the end of the submenu.
                } else if (knowx_islastFocusableElement(e.target, document.activeElement, focusSelector)) {
                    knowx_toggleSubMenu(e.target.parentNode, false);
                }
            }
        });
 ...
 */
function knowx_initNavToggleSmall() {
    ...
    for (let i = 0; i < navTOGGLE.length; i++) {
        knowx_initEachNavToggleSmall(navTOGGLE[i]);
    }
}
 ...
 * @param {Object} nav Navigation element.
 */
function knowx_initEachNavToggleSmall(nav) {
 ...
 * @return {void}
 */
function knowx_toggleSubMenu(parentMenuItem, forceToggle) {
    ...
    if (parentMenuItemToggled) {
        ...
        for (let i = 0; i < subMenuItemsToggled.length; i++) {
            knowx_toggleSubMenu(subMenuItemsToggled[i], false);
        }
    } else {
        ...
        for (let i = 0; i < parentMenuItemsToggled.length; i++) {
            knowx_toggleSubMenu(parentMenuItemsToggled[i], false);
        }
 ...
 */
function knowx_getDropdownButton() {
 ...
 */
function knowx_isfirstFocusableElement(container, element, focusSelector) {
 ...
 */
function knowx_islastFocusableElement(container, element, focusSelector) {
    var focusableElements = container.querySelectorAll(focusSelector);
    if (0 < focusableElements.length) {
...
(function() {
    ...
    var knowxHeader = document.getElementById('masthead'),
        mobnavWrap = document.querySelectorAll('.mobile-menu-container')[0];
    if (!knowxHeader || !mobnavWrap) {
        return;
    }
