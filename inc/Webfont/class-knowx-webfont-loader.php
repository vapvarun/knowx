...
class Knowx_WebFont_Loader {
	...
	public function __construct( $url = '' ) {
		...
		add_action( 'knowx_delete_fonts_folder', array( $this, 'knowx_delete_fonts_folder' ) );
	}
	...
	public function get_url() {
		...
		update_option( 'knowx_font_url', wp_json_encode( $google_font_url ) );
	...
	public function preload_local_fonts() {
		...
		update_site_option( 'knowx_local_font_files', $local_font );
	...
	public function get_local_stylesheet_filename() {
		return apply_filters( 'knowx_local_font_file_name', 'knowx-local-fonts' );
	}
	...
	public function set_font_format( $format = 'woff2' ) {
		$this->font_format = apply_filters( 'knowx_local_google_fonts_format', $format );
	}
	...
	public function get_base_path() {
		if ( ! $this->base_path ) {
			$this->base_path = apply_filters( 'knowx_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() . '/uploads/' );
		}
	...
	public function get_base_url() {
		if ( ! $this->base_url ) {
			$this->base_url = apply_filters( 'knowx_local_fonts_base_url', content_url() . '/uploads' );
		}
	...
	public function get_subfolder_name() {
		if ( ! $this->subfolder_name ) {
			$this->subfolder_name = apply_filters( 'knowx_local_fonts_directory_name', 'knowx-local-fonts' );
		}
	...
	public function schedule_cleanup() {
		if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
			if ( ! wp_next_scheduled( 'knowx_delete_fonts_folder' ) && ! wp_installing() ) {
				wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'knowx_delete_fonts_folder' );  // phpcs:ignore WPThemeReview.PluginTerritory.ForbiddenFunctions.cron_functionality_wp_schedule_event
			}
		}
	}
	 ...
	 * @return bool
	 */
	public function knowx_delete_fonts_folder() {
		// Delete previously created supportive options.
		delete_option( 'knowx_font_url' );
		delete_site_option( 'knowx_local_font_files' );
...
/**
 * Create instance of Knowx_WebFont_Loader class.
 *
 * @param string $font_url Google font URL to set data.
 * @return object
 * @since 1.5.0
 */
function knowx_webfont_loader_instance( $font_url = '' ) {
	return new Knowx_WebFont_Loader( $font_url );
}
 ...
 */
function knowx_load_preload_local_fonts( $url, $format = 'woff2' ) {

	// Check if cached font files data preset present or not. Basically avoiding 'Knowx_WebFont_Loader' class rendering.
	$knowx_local_font_files = get_site_option( 'knowx_local_font_files', false );

	if ( is_array( $knowx_local_font_files ) && ! empty( $knowx_local_font_files ) ) {
		$font_format = apply_filters( 'knowx_local_google_fonts_format', $format );
		foreach ( $knowx_local_font_files as $key => $local_font ) {
			if ( $local_font ) {
	...
	$font = knowx_webfont_loader_instance( $url );
 ...
 */
function knowx_get_webfont_url( $url, $format = 'woff2' ) {

	// Check if already Google font URL present or not. Basically avoiding 'Knowx_WebFont_Loader' class rendering.
	/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	$knowx_font_url = get_option( 'knowx_font_url', false );
	if ( $knowx_font_url ) {
		return json_decode( $knowx_font_url );
	}
 ...
 * @since 1.5.0
 */
function knowx_regenerate_fonts_folder() {

	if ( ! current_user_can( 'edit_theme_options' ) ) {
		wp_send_json_error( 'invalid_permissions' );
	}

	if ( get_option( 'knowx_font_url' ) ) {
		$local_font_loader = knowx_webfont_loader_instance( '' );
		$flushed           = $local_font_loader->knowx_delete_fonts_folder();
...
add_action( 'wp_ajax_knowx_regenerate_fonts_folder', 'knowx_regenerate_fonts_folder' );

/**
 *  Delete the cached partial configs.
 */
function knowx_delete_cached_partials() {

	// Delete previously stored local fonts data, if exists.
	if ( get_option( 'knowx_font_url' ) ) {
		$local_webfont_loader = knowx_webfont_loader_instance( '' );
		$local_webfont_loader->knowx_delete_fonts_folder();
	}
}
add_action( 'customize_save_after', 'knowx_delete_cached_partials' );
